...
namespace UnityEngine.UI.Extensions
{
	...
	[AddComponentMenu("Layout/Extensions/Horizontal Scroll Snap (\"Child Force Expand\" on LayoutGroup must be turned off)")]
	public class HorizontalScrollSnapMultiple : ScrollSnapBase, IEndDragHandler
	{
		[Tooltip("Number of children in a page")]
		[SerializeField]
		[Range(1, 10)]
		public int ChildsInPage = 1;
		...
		private void DistributePages()
		{
			...
			var pageStepValue = _childSize = (int)(panelDimensions.width/ ChildsInPage) * ((PageStep == 0) ? 3 : PageStep);

			for (int i = 0; i < _screensContainer.transform.childCount; i++)
			{
				RectTransform child = _screensContainer.transform.GetChild(i).gameObject.GetComponent<RectTransform>();
				currentXPosition = _offset + (int)(i * pageStepValue);
				child.sizeDelta = new Vector2(pageStepValue, panelDimensions.height);
